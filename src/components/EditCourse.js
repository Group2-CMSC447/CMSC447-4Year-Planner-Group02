...
function EditCourse({onConfirm}) { //Takes a prop from semester which is an on confirm event that calls add course
  ...
  const handleConfirm = () => { //runs after clicking add in the modal
      if (selectedCourse) { //ensures a course was actually clicked
          onConfirm(selectedCourse.name, selectedCourse.id, selectedCourse.credits, selectedCourse.workload); //uses the passed prop from semester to send the selected value to a function called add course which gets the selected value as a prop newCourse
